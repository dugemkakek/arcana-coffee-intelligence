...
- **local-node app** — Fastify backend + Next.js 15 + Electron + electron-builder
  ...
  - Detail page (`/roasts/[id]`) with BT/ET/RoR time-series chart and event markers
- **ai-service app** — standalone Fastify service on port 4001
  ...
  - Pluggable adapter system (MiniMax default, Anthropic + OpenRouter + local stubs)
  - Prompt templates in `apps/ai-service/src/prompts/`
...
- **packages/ai-adapters** — `AIProvider` interface + MiniMax implementation + Anthropic + OpenRouter stubs
